...
# trainer.py : trainer program and basic result visualization of dna-circuit 
# convolution classification
# 
# this program is for cpu only. the training task is simple and capable for cpu 
# performance.
...
def eval_model(testImages, testTags, modulePath, cpid, networkid):
    ...
    # statedict['conn1.bias'] /= 28.6339
    # statedict['conn2.bias'] /= 28.6339 # 60.1970
...
class Convs4x44 (torch.nn.Module):
    ...
    def __init__(self):
        ...
        # self.leakyrelu1 = torch.nn.LeakyReLU(0)
        self.bn2d = torch.nn.BatchNorm2d(1)
        self.bn1d4 = torch.nn.BatchNorm1d(1)
        self.bn1d3 = torch.nn.BatchNorm1d(1)
    ...
    def forward(self, x):
        ...
        for batchId in range(batch):
            ...
            inp_fullconn = (inp_fullconn - 2).relu()
            inp_fullconn = inp_fullconn # / 3.4859
            
            inp_interm = self.conn1(inp_fullconn)
            inp_interm = (inp_interm).relu()
            inp_interm = inp_interm # / 2.1023
